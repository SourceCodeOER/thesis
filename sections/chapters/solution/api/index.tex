...
\begin{itemize}
    ...
    \item .github : nos scripts d'automatisation (cf. section \ref{section:GithubActions}) 
...
Nous avons ainsi découpé notre application en 3 images\footnote{
    ...
    et générés sur base de fichiers Dockerfile, présent dans chaque repository Github.
    Notons enfin la présence dans le repository miscellaneous de fichiers docker-compose-***.yml pour déployer automatiquement l'application avec Docker Compose, en fonction de l'environnement souhaité (plus d'informations dans le README.MD).
} :
