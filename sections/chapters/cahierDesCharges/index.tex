...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Rechercher les \glspl{fiche} par leurs identifiants
        \item Rechercher les \glspl{fiche} par leurs créateurs
        \item Rechercher les \glspl{fiche} par leurs états
        \item Rechercher les \glspl{fiche} sur base d'un certain seuil sur le résultat moyen accordé par les utilisateurs
    \end{itemize}
...
Un application web à destination d'un public varié et conséquent se doit d'avoir une interface simple et efficace. On peut considérer que le design d'une application est subjectif d'une certaine manière, mais un point à ne sûrement pas négliger se situe autour de l'ergonomie. La question à se poser pour chaque interface est alors de savoir comment disposer les éléments afin de rendre la navigation la plus claire et pratique possible. Nous nous sommes donc concentrés sur cette problématique en élaborant un patchwork que vous pouvez consulter à la fin de la section \ref{section:problem}\\
...
Une première contrainte évidente est indubitablement le temps. Nous avons consacré année académique entière sur ce mémoire mais ce n'est certainement pas assez pour rendre mature \texttt{SourceCode}.\\
