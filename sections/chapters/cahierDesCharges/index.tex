...
\begin{itemize}
    ...
    \end{itemize}
    \item Ordonner les \glspl{fiche} lors de la recherche avec une libre combinaison des paramètres suivants (en précisant pour chacun l'ordre du tri : ascendant / descendant) :
            \begin{itemize}
        ...
        \end{itemize}
    \item Choisir quelles propriétés des \glspl{fiche} que l'on souhaite consulter
    \item Consulter une \gls{fiche}
    \item Consulter les \glspl{tag} et \glspl{tagCat} existants (optionnellement avec des statistiques d'utilisation de ceux-ci)
...
\begin{itemize}
    ...
    \item Modifier les informations d'une \gls{fiche}  lui appartenant
    \item Changer l'état d'une \gls{fiche}  lui appartenant
    \item Proposer un nouveau ou plusieurs mot(s) clé(s)
    \item Évaluer une \gls{fiche}  
    \item Gestion de favoris
    \item Utilisation des favoris dans la recherche de \gls{fiche}s
\end{itemize}
